    ...
    employeeTable = $('#employee_table').DataTable({
        ...
        columns : [
            ...
            { data : null, orderable : false, searchable : false, render : function(data, type, row){
                return '<button onclick="openEditModal(' + row.id + ')" title="Edit" class="text-blue-800 hover:text-white hover:bg-blue-800 p-2 rounded-lg transition">'
                    + '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>'
                    + '</button>'
                    + '<button onclick="deleteEmployee(' + row.id + ')" title="Delete" class="text-red-700 hover:text-white hover:bg-red-700 p-2 rounded-lg transition">'
